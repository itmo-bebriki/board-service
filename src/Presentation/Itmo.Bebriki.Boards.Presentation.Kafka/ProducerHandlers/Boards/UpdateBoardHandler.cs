...
internal sealed class UpdateBoardHandler : IEventHandler<UpdateBoardEvent>
{
    private readonly IKafkaMessageProducer<BoardInfoKey, BoardInfoValue> _producer;

    public UpdateBoardHandler(IKafkaMessageProducer<BoardInfoKey, BoardInfoValue> producer)
    {
        _producer = producer;
    }

    public async ValueTask HandleAsync(UpdateBoardEvent evt, CancellationToken cancellationToken)
    {
